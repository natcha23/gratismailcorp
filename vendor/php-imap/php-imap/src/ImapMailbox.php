...
class ImapMailbox {
	...
	protected function initImapStream() {
		...
		if(!$imapStream) {
			...
			if(strpos(imap_last_error(), 'AUTHENTICATIONFAILED')):
				return ['error' => ['message' => 'Please re-check your email and password.']];
			else:
				return true;
			endif;
		}
	...
	public function searchMailBoxNoCharset($criteria = 'ALL') {
	...
	public function deleteMail($mailId) {
		if(imap_delete($this->getImapStream(), $mailId, FT_UID))
			return true;
		else
	 ...
	 * @return IncomingMail
	 */
	public function getMailList($skip = 0, $limit = ''){
		$skip = $skip + 1;
		if(empty($limit)):
			$mc = imap_check($this->getImapStream());
			$limit = $mc->Nmsgs;
		endif;
		$mails = imap_fetch_overview($this->getImapStream(), "$skip:{$limit}", FT_UID);
		if(!$mails)
			return 0;
		$results = '';
	...
	protected function initMailPart(IncomingMail $mail, $partStructure, $partNum) {
		...
		if($attachmentId) {
			...
			if($this->attachmentsDir) {
				...
				$attachment->fullName = $fileSysName;
			}
