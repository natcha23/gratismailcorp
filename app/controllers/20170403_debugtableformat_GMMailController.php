...
/////////////////
// 			$username = Session::get('logged_in')->username; //smmcorporate@ais.co.th
// 			$password = Session::get('logged_in')->password; //Ais@12Nov2015
// 			@imap_open("{mailaccess.ais.co.th:110/pop3}INBOX", $username, $password, OP_DEBUG, 1) or die('Cannot connect: ' . _print(imap_errors(), true));
// 			$connection = @imap_open("{mailaccess.ais.co.th:110/pop3}INBOX", 'smmcorporate@ais.co.th', 'Ais@12Nov2015', 0, 0, '');
// 			$connection = @imap_open("{mailaccess.ais.co.th:110/pop3}INBOX", "smmcorporate@ais.co.th", "Ais@12Nov2015");
/////////////////
			try {
	...
	// 			echo "Caught exception : <b>".$e->getMessage()."</b><br/>";
		...
		if(Session::get('logged_in')->username == 'smmcorporate@ais.co.th') {
			_print(htmlspecialchars($mail->text));
		}
		if($mail->folder_id == 4):
...
// 			$mail->text = substituteHtml($mail->text);
			// function substituteHtml()
			$data = $mail->text;
// 			_print(htmlspecialchars($data));
			$findString = '<td';
			$count = substr_count($data, $findString);
			if($count > 0) {
				$pattern = "!<td(.*?)>(.*?)<td(.*?)>!is";
				$mpmatch = "/<td(.*?)>(.*?)<td(.*?)>/s";
				$notmatch = "/<td(.*?)>(.*?)<\/td>/s";
				$pos = strpos($data, $findString);
					
				preg_match_all($mpmatch, $data, $matches);
				
// 				_print($matches[0]);
				foreach($matches[0] as $element) {
					echo '<pre>'.print_r(htmlspecialchars($element),1).'</td>'. '</pre>';
					$newdata = preg_replace  ( $mpmatch, $element.'</td>', $element );
				}
				
				_print(htmlspecialchars($newdata));
// 				preg_match($mpmatch, $data, $result);
				
// 				$html = "<td>sdfdfdfdsfds</td><td>333333333</td>";
...
exit;
		...
		$mail->text = $mail->text."<table><tbody><tr valign='top' height='8'><br /><td>22222222222</tr><tr valign='top' height='8'><br /><td>44444444444</tr></table>";
		Session::forget('mail_draft');
