...
class GMCallerController extends BaseController {
	...
	* - URL: http://10.239.252.63/SMMGetInfo/gratismailcorp/index.php/caller/get/emails/inbox?u=MTE0OXRlc3Qx&p=M1VQTjRyQDVZYiNl&reset=users
	*/
		...
		if(!$mailsIds):
			...
			foreach($mails as $mail):
				...
				if(!$mail_in_db):
					...
					/* ignore email lists */
					$ignore_lists = array('corporatecallcenter@ais.co.th');
								...
								foreach($attachments as $fid => $attachment):
									try{
										GMAttachment::insert([
											'mail_id' => $mail_id,
											'fid' => $fid,
											'file_name' => date('Ymd') . '/' . urlencode($attachment->fullName),
											'file_name_original' => date('Ymd') . '/' . urlencode($attachment->name),
											'created_at' => date('Y-M-d H:i:s'),
											'updated_at' => date('Y-M-d H:i:s'),
										]);
									} catch (Exception $e) {
