...
class GMMailController extends BaseController {
    ...
    if(!empty($data['files'])):
      ...
      foreach($attachments as $fid):
        ...
        if($mail_id_current == $file->mail_id):
          $old = public_path('attachments/'. date('Y-m-d') . '/' . $file->file_name);
          $new = public_path('attachments/'. date('Y-m-d') . '/' . $file_name);
          copy($old, $new);
          GMAttachment::insert([
            'mail_id' => $mail_id,
            'fid' => $fid,
  ...
  // }
    ...
    {
        $pathForCheck = '/var/www/html/SMMGetInfo/gratismailcorp/public/attachments/' . date('Y-m-d') . '/';
